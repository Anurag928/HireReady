...
function SidebarContent({ onClose, isCollapsed = false, toggleCollapse }: { onClose?: () => void; isCollapsed?: boolean; toggleCollapse?: () => void }) {
  ...
  return (
    <div className="flex flex-col h-full bg-white dark:bg-[#0b1220]/80 border-r border-black/10 dark:border-white/10 shadow-[20px_0_50px_rgba(0,0,0,0.02)] dark:shadow-[20px_0_50px_rgba(0,0,0,0.5)] transition-colors duration-300 relative">
      
      {/* Toggle Button */}
      {toggleCollapse && (
        <button
          onClick={toggleCollapse}
          className="absolute -right-3 top-8 z-50 p-1.5 rounded-full bg-white dark:bg-[#1a2333] border border-black/10 dark:border-white/10 shadow-sm text-zinc-500 dark:text-zinc-400 hover:text-zinc-900 dark:hover:text-white transition-all hover:scale-110 flex items-center justify-center backdrop-blur-md"
        >
      ...
      {/* Logo Hub */}
      <div className={`h-20 lg:h-24 flex items-center ${isCollapsed ? 'justify-center px-0' : 'justify-between px-6 lg:px-8'} shrink-0 relative transition-all duration-300`}>
        <Link href="/" className="flex items-center gap-3 group overflow-hidden" onClick={onClose}>
      ...
      <AnimatePresence>
        {!isCollapsed && (
          <motion.div 
            ...
            className="px-4 lg:px-6 overflow-hidden shrink-0"
          >
      ...
      {/* Navigation Grid */}
      <nav className="flex-1 px-3 lg:px-4 space-y-1 overflow-y-auto custom-scrollbar overflow-x-hidden">
        {navItems.map((item) => {
          ...
          return (
            <Link
              ...
              className={`group relative flex items-center justify-between ${isCollapsed ? 'justify-center px-0 w-12 mx-auto' : 'px-4 lg:px-5'} py-3 lg:py-2.5 rounded-2xl transition-all duration-300 ${isActive
            ...
            >
              ...
              <div className="flex items-center gap-4 relative z-10 overflow-hidden">
                <div className={`p-1.5 shrink-0 rounded-xl transition-colors ${isActive ? "bg-blue-600 text-white dark:bg-cyan-500 dark:text-black" : "bg-black/5 dark:bg-white/10 text-zinc-600 dark:text-zinc-400 group-hover:text-zinc-900 dark:group-hover:text-white group-hover:bg-black/10 dark:group-hover:bg-white/20"}`}>
                ...
                <AnimatePresence>
                  {!isCollapsed && (
                    <motion.span 
                      ...
                      animate={{ opacity: 1, width: "auto" }}
                      exit={{ opacity: 0, width: 0 }}
                      className={`text-[10px] font-black uppercase tracking-widest whitespace-nowrap transition-all ${isActive ? "text-blue-700 dark:text-cyan-300" : "text-zinc-600 dark:text-zinc-400 group-hover:text-zinc-900 dark:group-hover:text-white group-hover:translate-x-1"}`}
                    >
                      {item.name}
                    </motion.span>
                  )}
              ...
              <AnimatePresence>
                {!isCollapsed && item.badge && (
                   <motion.div 
                     ...
                     className={`relative z-10 shrink-0 px-2.5 py-0.5 rounded-full border text-[8px] font-black uppercase tracking-widest ${isActive ? 'bg-blue-500/10 border-blue-500/20 text-blue-600 dark:text-cyan-300 dark:border-cyan-400/30' : 'bg-black/5 dark:bg-white/5 border-black/10 dark:border-white/10 text-zinc-500 dark:text-zinc-400'}`}
                   >
                      {item.badge}
                   </motion.div>
                )}
      ...
      {/* User Hub (Bottom) */}
      <div className={`p-4 border-t border-black/10 dark:border-white/10 space-y-3 shrink-0 overflow-x-hidden flex flex-col transition-all duration-300 ${isCollapsed ? 'items-center px-2' : ''}`}>
        <Link 
          href="/profile"
          onClick={onClose}
          title={isCollapsed ? "Profile" : undefined}
          className={`flex items-center gap-3 ${isCollapsed ? 'justify-center px-0 w-12' : 'px-2'} hover:bg-black/5 dark:hover:bg-white/5 py-1.5 rounded-2xl transition-all overflow-hidden`}
        >
           <div className="w-8 h-8 shrink-0 rounded-xl bg-gradient-to-br from-zinc-200 to-zinc-300 dark:from-zinc-800 dark:to-black border border-black/10 dark:border-white/10 overflow-hidden p-0.5">
        ...
        <button
          ...
          className={`group flex items-center gap-3 py-2.5 rounded-2xl text-rose-600 dark:text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-500/10 transition-all font-black uppercase tracking-[0.2em] text-[10px] overflow-hidden ${isCollapsed ? 'justify-center px-0 w-12' : 'px-4 w-full'}`}
        >
          <div className="p-1.5 shrink-0 rounded-lg bg-rose-100 dark:bg-rose-500/5 border border-rose-200 dark:border-rose-500/10 group-hover:bg-rose-500 group-hover:text-white transition-all">
  ...
  return (
    <div className="flex h-[100dvh] bg-[#f8fafc] dark:bg-[#050816] text-zinc-900 dark:text-white selection:bg-blue-500/30 overflow-hidden transition-colors duration-300">
      ...
      <motion.aside 
        ...
        className="hidden lg:block relative z-50 transition-colors duration-300 flex-shrink-0"
      >
        <SidebarContent 
